...
export default async function CreatorProfilePage({ params }: Props) {
  const { data: p } = await getProviderBySlug(params.slug);
  if (!p || p.type !== 'creator') notFound();
  const catObj = CREATOR_CATEGORIES.find(c => c.slug === p.category_slug);
    ...
    <>
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '40px 24px' }}>
        <p style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 12, color: '#7A7773', marginBottom: 24 }}>
          <Link href="/creators" style={{ color: '#7A7773', textDecoration: 'none' }}>Creators</Link>
          {catObj && <>{' / '}<Link href={`/creators/${p.category_slug}`} style={{ color: '#7A7773', textDecoration: 'none' }}>{catObj.name}</Link></>}
          {' / '}{p.name}
        </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 40 }}>
          <div>
            ...
            <p style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 13, color: '#7A7773', marginBottom: 20 }}>{p.category}</p>
            ...
            {p.services && p.services.length > 0 && (
              <div style={{ marginBottom: 28 }}>
                <p style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 10, color: '#7A7773', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 12 }}>Content areas</p>
            ...
            {p.verified ? (
              <div style={{ background: '#0A0908', padding: '24px', marginTop: 16 }}>
                ...
                <p style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: 16, color: '#F8F6F0', marginBottom: 10, lineHeight: 1.5 }}>
                  This creator has been independently verified by Few Found.
                </p>
                {p.verification_date && (
              ...
              <div style={{ border: '1px solid #E2DED6', padding: '22px', marginTop: 16 }}>
                ...
                <p style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 13, color: '#3D3B38', marginBottom: 14, lineHeight: 1.6 }}>
                  Verification confirms audience authenticity, engagement quality, and brand collaboration references.
                </p>
                <Link href="/verify?type=creator" style={{ background: '#0A0908', color: '#F8F6F0', padding: '10px 18px', textDecoration: 'none', fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 12, fontWeight: 600, display: 'inline-block' }}>
                  Apply for verification  -  ₹14,999/year
                </Link>
              </div>
            )}
          </div>
          ...
          <aside>
            <div style={{ border: '1px solid #E2DED6', padding: '20px', marginBottom: 14 }}>
              ...
              {[{ label: 'Type', val: 'Content creator' }, { label: 'Category', val: p.category }].filter(r => r.val).map(r => (
              ...
              {p.website && (
                <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid #E2DED6' }}>
                  <a href={p.website} target="_blank" rel="noopener noreferrer" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 12, color: '#0A0908', textDecoration: 'underline' }}>Visit channel / page</a>
                </div>
              )}
            </div>
            <div style={{ background: '#0A0908', padding: '20px' }}>
              ...
              <p style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: 14, color: '#F8F6F0', marginBottom: 10, lineHeight: 1.5 }}>Brand members see score bands, engagement audits, and reference contacts.</p>
