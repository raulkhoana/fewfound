...
export async function POST(req: NextRequest) {
  try {
    ...
    await resend.emails.send({
      ...
      html: `<h2>New Brand Review</h2><p><strong>Brand:</strong> ${brand_name}</p><p><strong>Reviewer role:</strong> ${reviewer_role}</p><p><strong>Payment timing:</strong> ${payment_timing}</p><p><strong>Brief quality:</strong> ${brief_quality}</p><p><strong>Approval speed:</strong> ${approval_speed}</p><p><strong>Would work again:</strong> ${would_work_again}</p><p><strong>Comment:</strong> ${comment || 'None'}</p>`,
    }).catch(() => {});
